...
public class ValidityService extends Service implements FingerprintCore.EventListener {
    ...
    public void onCreate() {
        ...
        try {
            ...
            VLog.i("in onCreate, making server socket: " + e);
            return;
        }
        Thread t_server = new Thread() {
            ...
            public void run() {
                ...
                while (true) {
                    try {
                        VLog.i("Waiting for connection...");
                        socket = mServerSocket.accept();
                        VLog.i(".....Got socket: " + socket);
                        if (socket != null) {
                            startService(socket);
                        } else {
                            return;
    ...
    public void onEvent(final FingerprintEvent event) {
        VLog.i("identify onEvent: receive event :" + event.eventId);
        ...
        try {
            ...
            switch (event.eventId) {
                ...
                case VcsEvents.VCS_EVT_IDENTIFY_FAILED:
                     ...
                     break;
                default:
                     VLog.i("identify onEvent: No need to process event :" + event.eventId);
            }
            if (str != null) {
                osr.write(str);
                osr.flush();
