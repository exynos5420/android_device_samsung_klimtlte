...
public class SamsungExynosRIL extends RIL {
    ...
    public SamsungExynosRIL(Context context, int preferredNetworkType, int cdmaSubscription) {
    ...
    // Set newResponse to null if you dont need to remap the response.
    public void
    superProcessUnsolicited(Parcel p, int type, int dataPosition, int newResponse) {
        ...
        if (newResponse != null) { // if newResponse is not null we need to remap the response.
    ...
    processUnsolicited(Parcel p, int type) {
        ...
        if (response >= SAMSUNG_UNSOL_RESPONSE_BASE) {
            ...
            // Not an OEM response.
            superProcessUnsolicited(p, type, dataPosition, null);
            return;
        }
    }
}
